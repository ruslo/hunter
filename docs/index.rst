...
highly customizable effectively allowing you to have myriad variants of directories with them based on combinations of version to build, static/shared, CMake ``-D`` options, Release/Debug etc.
Separate root
...
Client is a collection of CMake only modules (i.e. it's **not a binary** like ``apt-get`` or script like ``brew``) so it supports from the box all
